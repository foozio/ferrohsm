...
use cryptoki_sys::{
    CK_INFO, CK_RV, CK_SLOT_ID, CK_SLOT_INFO, CK_TOKEN_INFO, CK_VOID_PTR,
    CKR_CRYPTOKI_ALREADY_INITIALIZED, CKR_CRYPTOKI_NOT_INITIALIZED, CKR_FUNCTION_FAILED,
    CKR_OK, CKR_ARGUMENTS_BAD, CK_SLOT_ID_PTR, CK_ULONG, CK_ULONG_PTR, CKR_BUFFER_TOO_SMALL,
    CK_SESSION_HANDLE, CK_SESSION_HANDLE_PTR, CK_FLAGS, CK_STATE, CK_USER_TYPE,
    CK_NOTIFICATION, CK_CHAR_PTR, CK_ATTRIBUTE_PTR, CK_MECHANISM_PTR, CK_OBJECT_HANDLE,
...
pub fn get_slot_list(_token_present: bool) -> Result<Vec<CK_SLOT_ID>, FrontendError> {
    ...
    let context = guard.context.as_ref().ok_or(FrontendError::NotInitialized)?;
    let context = context.lock().map_err(|_| FrontendError::Internal("Failed to lock context".to_string()))?;
...
pub fn get_slot_info(slot_id: CK_SLOT_ID) -> Result<CK_SLOT_INFO, FrontendError> {
    ...
    // Find the slot
    let slot = context.slots.iter().find(|slot| slot.id == slot_id)
...
pub fn open_session(slot_id: CK_SLOT_ID, flags: CK_FLAGS) -> Result<CK_SESSION_HANDLE, FrontendError> {
    let mut guard = STATE.write();
    let context = guard.context.as_mut().ok_or(FrontendError::NotInitialized)?;
    let mut context = context.lock().map_err(|_| FrontendError::Internal("Failed to lock context".to_string()))?;
    
    // Verify the slot exists
    if !context.slots.iter().any(|slot| slot.id == slot_id) {
        return Err(FrontendError::Internal(format!("Slot {} not found", slot_id)));
    }
    ...
    let state = if (flags & 0x00000004) != 0 { // CKF_RW_SESSION
...
pub fn close_session(session_handle: CK_SESSION_HANDLE) -> Result<(), FrontendError> {
    ...
    if context.sessions.remove(&session_handle).is_some() {
        ...
        context.objects.retain(|_, object| object.session_handle != session_handle);
        info!("Closed session {}", session_handle);
        Ok(())
    } else {
        Err(FrontendError::Internal(format!("Session {} not found", session_handle)))
    }
}
...
pub fn close_all_sessions(slot_id: CK_SLOT_ID) -> Result<(), FrontendError> {
    ...
    context.sessions.retain(|_, session| session.slot_id != slot_id);
...
/// Login to a session
pub fn login(session_handle: CK_SESSION_HANDLE, user_type: CK_USER_TYPE, pin: &[u8]) -> Result<(), FrontendError> {
    let mut guard = STATE.write();
    let context = guard.context.as_mut().ok_or(FrontendError::NotInitialized)?;
    let mut context = context.lock().map_err(|_| FrontendError::Internal("Failed to lock context".to_string()))?;
    
    // Find the session
    let session = context.sessions.get_mut(&session_handle)
    ...
    session.state = match user_type {
        0 => { // CKU_SO (Security Officer)
            if (session.flags & 0x00000004) != 0 { // CKF_RW_SESSION
                3 // CKS_RW_SO_FUNCTIONS
            } else {
                2 // CKS_RO_SO_FUNCTIONS (should not happen in practice)
            }
        }
        1 => { // CKU_USER (Regular user)
            if (session.flags & 0x00000004) != 0 { // CKF_RW_SESSION
        ...
        _ => {
            ...
            return Err(FrontendError::Internal(format!("Unsupported user type: {}", user_type)));
        }
    };
    
    info!("User logged in to session {} as type {}", session_handle, user_type);
    Ok(())
}
...
pub extern "C" fn C_Login(
    ...
    let pin_bytes = if ulPinLen == 0 {
        ...
        unsafe {
            std::slice::from_raw_parts(pPin, ulPinLen as usize).to_vec()
        }
    };
...
pub fn logout(session_handle: CK_SESSION_HANDLE) -> Result<(), FrontendError> {
    ...
    session.state = if (session.flags & 0x00000004) != 0 { // CKF_RW_SESSION
        1 // CKS_RW_PUBLIC_SESSION
    } else {
        0 // CKS_RO_PUBLIC_SESSION
    };
...
pub fn generate_key(
    ...
    // Find the session
    let slot_id = {
        let session = context.sessions.get(&session_handle)
            .ok_or_else(|| FrontendError::Internal(format!("Session {} not found", session_handle)))?;
    ...
    context.objects.insert(object_handle, ObjectInfo {
...
pub fn generate_key_pair(
    ...
    context.objects.insert(public_handle, ObjectInfo {
        handle: public_handle,
        session_handle,
        slot_id,
    });
    context.objects.insert(private_handle, ObjectInfo {
...
pub fn destroy_object(
    ...
    // Find the object
    let object = context.objects.get(&object_handle)
    ...
    if object.session_handle != session_handle {
        return Err(FrontendError::Internal("Object does not belong to this session".to_string()));
    }
    
    // Remove the object
    context.objects.remove(&object_handle);
    
    info!("Destroyed object {} in session {}", object_handle, session_handle);
    Ok(())
}
...
pub fn find_objects(
    ...
    let search_context = session.search_context.as_mut()
...
pub extern "C" fn C_FindObjects(
    ...
    match find_objects(hSession, &mut object_handles, ulMaxObjectCount, &mut object_count) {
        Ok(()) => {
...
pub fn get_session_info(session_handle: CK_SESSION_HANDLE) -> Result<(CK_SLOT_ID, CK_FLAGS, CK_STATE), FrontendError> {
    let guard = STATE.read();
    let context = guard.context.as_ref().ok_or(FrontendError::NotInitialized)?;
    let context = context.lock().map_err(|_| FrontendError::Internal("Failed to lock context".to_string()))?;
